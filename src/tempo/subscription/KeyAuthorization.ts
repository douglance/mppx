...
import type { SubscriptionAccessKey, SubscriptionCredentialPayload } from './Types.js'
 ...
 * authorizations.
 */
export function toSubscriptionExpirySeconds(subscriptionExpires: string): number {
  const milliseconds = new Date(subscriptionExpires).getTime()
...
/**
 * Converts the spec string `periodSeconds` field into the numeric period accepted by Tempo key
 * authorizations.
 */
export function toSubscriptionPeriodSeconds(periodSeconds: string): number {
  if (!/^[1-9]\d*$/.test(periodSeconds)) {
    throw new VerificationFailedError({ reason: 'periodSeconds is invalid' })
  }

  const value = BigInt(periodSeconds)
  if (value > uint64Max) {
    throw new VerificationFailedError({
      reason: 'periodSeconds cannot be represented as an unsigned 64-bit integer',
    })
  }
  if (value > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new VerificationFailedError({
      reason: 'periodSeconds cannot be represented exactly by this Tempo client',
    })
  }
...
export function assertSubscriptionTiming(parameters: {
  ...
  request: Pick<SubscriptionRequest, 'periodSeconds' | 'subscriptionExpires'>
}) {
  const { challengeExpires, request } = parameters
  toSubscriptionPeriodSeconds(request.periodSeconds)
  const subscriptionExpiry = toSubscriptionExpirySeconds(request.subscriptionExpires)
...
function assertAuthorizationExpiry(
  ...
  if (authorization.expiry !== toSubscriptionExpirySeconds(request.subscriptionExpires)) {
...
function assertAuthorizationLimit(
  ...
  request: Pick<SubscriptionRequest, 'amount' | 'currency' | 'periodSeconds'>,
) {
  ...
  if (limit.period !== toSubscriptionPeriodSeconds(request.periodSeconds)) {
