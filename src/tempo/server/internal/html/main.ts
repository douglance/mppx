...
const buttonLabel =
  c.challenge.intent === 'subscription'
    ? 'Authorize with'
    : c.text.pay === 'Pay'
      ? 'Continue with'
      : c.text.pay
...
button.onclick = async () => {
  try {
    ...
    const credential = await (async () => {
      if (c.challenge.intent === 'charge') {
        const challenge = c.challenge as Extract<typeof c.challenge, { intent: 'charge' }>
        const method = tempo.charge({ account, getClient })
        return method.createCredential({ challenge, context: {} })
      }

      if (c.challenge.intent === 'subscription') {
        if (!c.config.accessKey) throw new Error('Missing subscription access key configuration.')

        const challenge = c.challenge as Extract<typeof c.challenge, { intent: 'subscription' }>
      ...
      throw new Error(`Unsupported Tempo HTML intent: ${c.challenge.intent}`)
    })()
