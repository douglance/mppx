...
Recurring access-key subscription for a news app. The server charges `0.000001` pathUSD per second by resolving the user to `{ key, accessKey }`, returning that dynamic Tempo access key in the MPP challenge, then requiring a `keyAuthorization` scoped to that key.
...
3. Waits long enough for the one-second billing period to elapse.
4. Requests `/api/article` again, causing the server to renew the subscription with the same access key.
